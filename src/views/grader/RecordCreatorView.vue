...
<template>
    <RecordLab v-if="state.rubricReady.value" :rubric-group="state.currentRubricGroupSelection.value" :team="state.currentTeamSelection.value"
               mode="create" @create="handleNewRecordSubmission"/>
    <Dialog :dialog-open="!state.rubricReady.value" action-button-label="Create Rubric"
            :action-button-handler="() => prepareRubric(state.currentTeamSelection.value?.teamNumber, state.currentRubricGroupSelection.value?.id)" :action-button-handler-disabled="rubricCreatorDisabled"
            @close="handleTeamSelectorDialogClose">
      <VerticalStack spacing="lg">
        <TeamSelector v-model:team-selection="state.currentTeamSelection.value"
                      :team-selections="state.teamSelections.value"/>
        <RubricGroupSelector v-model:rubric-group-selection="state.currentRubricGroupSelection.value" :rubric-group-selections="rubricGroupSelections" />
      </VerticalStack>
    </Dialog>
    <StatusDialog v-if="!!state.statusDialogState.value" :type="state.statusDialogState.value?.type"
                  :message="state.statusDialogState.value?.message" v-model:dialog-open="state.statusDialogOpen.value"
